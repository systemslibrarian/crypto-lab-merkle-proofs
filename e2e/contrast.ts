 ...
 *    files the node under `incomplete`. The chips and verdict panels here are
 *    translucent tints over the card, so their text composites onto something
 *    axe will not measure.
 ...
 *    animations to 0.01ms and scopes the pulsing node to `no-preference` —
 *    both safe patterns, so nothing strands at `opacity: 0`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The tree canvas
 *    and the 64-hex digest rows overflow their cards at 380px, so much of them
 *    is clipped away. Content scrolled past the client box is not dimmed or partly
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. The tree
 *    figure draws each node as a `<rect>` and then its label as a following
 *    `<text>`, so a label's backdrop is a PRECEDING SIBLING shape, not an
 *    ancestor. An ancestor-only
