 ...
 *    animations to `0.01ms` rather than cancelling them and scopes the pulsing
 *    node to `no-preference` — both safe patterns, so nothing strands at
 *    `opacity: 0` under the preference.
 *
 ...
 *    the tree canvas and the 64-hex digest rows overflow their cards at 380px,
 *    so much of them is clipped away. Content scrolled past the client box is not dimmed or partly
