 ...
 *     nothing, and each of this lab's eight exhibits writes into an output
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#root-hash')).not.toBeEmpty();
  await expect(page.locator('#verify-run')).toBeVisible();
 ...
 * plausible offender: it draws a tree canvas, prints 64-hex node digests, and
 * lays exhibits out on `repeat(auto-fit, minmax(220px, 1fr))` tracks whose
 * fixed floor a 380px viewport cannot honour.
 */
 ...
 *
 * Both branches are driven wherever an exhibit has them: an honest proof and a
 * tampered one are different verdict surfaces with different palettes, and it
 * is the failure branch that a gate scanning only first paint never reaches.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  // Build a tree from the sample preset, then add a leaf by hand.
  await page.locator('#preset-sample').click();
  await expect(page.locator('#root-hash')).not.toBeEmpty();
  await scan(page, `${theme} / sample tree built`);

  await page.locator('#leaf-input').fill('extra-leaf');
  await page.locator('#leaf-add').click();
  await scan(page, `${theme} / leaf appended`);

  // Inclusion proof: the honest path, then the tampered one.
  await page.locator('#verify-run').click();
  await expect(page.locator('#verify-verdict')).not.toBeEmpty();
  await scan(page, `${theme} / inclusion proof verified`);

  await page.locator('#verify-tamper-leaf').click();
  await expect(page.locator('#verify-verdict')).not.toBeEmpty();
  await scan(page, `${theme} / inclusion proof tampered`);

  await page.locator('#verify-honest').click();
  await page.locator('#verify-step').click();
  await expect(page.locator('#verify-step-status')).not.toBeEmpty();
  await scan(page, `${theme} / proof step-through`);

  // The remaining exhibits, each with its failure branch where it has one.
  await page.locator('#dup-run').click();
  await expect(page.locator('#dup-output')).not.toBeEmpty();
  await scan(page, `${theme} / duplicate-leaf attack`);

  await page.locator('#sec-run').click();
  await expect(page.locator('#sec-output')).not.toBeEmpty();
  await scan(page, `${theme} / second-preimage`);

  await page.locator('#ct-run').click();
  await expect(page.locator('#ct-output')).not.toBeEmpty();
  await scan(page, `${theme} / certificate transparency`);

  await page.locator('#ct-tamper').click();
  await expect(page.locator('#ct-output')).not.toBeEmpty();
  await scan(page, `${theme} / CT tampered`);

  await page.locator('#cons-run').click();
  await expect(page.locator('#cons-output')).not.toBeEmpty();
  await scan(page, `${theme} / consistency proof`);

  await page.locator('#cons-tamper-reorder').click();
  await expect(page.locator('#cons-output')).not.toBeEmpty();
  await scan(page, `${theme} / consistency reorder detected`);

  await page.locator('#trust-attack').click();
  await expect(page.locator('#trust-output')).not.toBeEmpty();
  await scan(page, `${theme} / trust attack`);

  await page.locator('#eff-slider').fill('12');
  await expect(page.locator('#eff-readout')).not.toBeEmpty();
  await scan(page, `${theme} / proof-size efficiency`);
}
